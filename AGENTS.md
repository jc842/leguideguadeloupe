# Directives & Contraintes d'Environnement — leguideguadeloupe.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **leguideguadeloupe.com** (leguideguadeloupe) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `leguideguadeloupe.com` |
| **URL Cloudflare Pages** | `https://leguideguadeloupe.pages.dev` |
| **Dépôt GitHub** | `jc842/leguideguadeloupe` (branche principale : `main`) |
...
| **Matomo Site ID** | `107` |
| **Matomo MTM Container** | `container_3Jk5GcF6.js` (Container ID : `3Jk5GcF6`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `112` |
| **Zones Publicitaires** | Header (`672`), In-Content (`673`), Mobile Sticky (`674`), Large Rectangle (`671`), Half Page (`670`), Skyscraper (`675`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_3Jk5GcF6.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 673).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 671).
